...
@if ($crud->fieldTypeNotLoaded($field))
  ...
  @push('crud_fields_scripts')
      <script>
          $(document).on('change', '[data-repeatable-input-name="item_id"]', function() {
              $.ajax({
                  url: '/api-admin/get-item-detail?item_id=' + $(this).val(),
                  method: 'GET',
                  success: function(response) {
                      var html = '';
                      var thumbnail = response['data'][0]['thumbnail'];
                      html += '<img src="/' + thumbnail + '">';
                      $("#preview-item .image").html(html);
                  },
              });
          });
          $(document).on('change', '[data-repeatable-input-name="item_id[]"]', function() {
              $.ajax({
                  url: '/api-admin/get-items-preview?arr_item_id=' + $(this).val(),
                  method: 'GET',
                  success: function(response) {
                      var html = '';
