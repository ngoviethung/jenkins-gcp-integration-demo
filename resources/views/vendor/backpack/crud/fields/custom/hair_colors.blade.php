...
@php
  ...
  $total_color = count($hair_items);

  $field['init_rows'] = $total_color;
  $field['max_rows'] = $total_color;
  $field['min_rows'] =  $total_color;
@endphp
...
<div class="container-repeatable-elements">
    ...
    @push('before_scripts')
    <div class="col-md-6 well repeatable-element row m-1 p-2" data-repeatable-identifier="{{ $field['name'] }}">
  ...
  @push('crud_fields_scripts')
      <script>
        ...
        function repeatableInputToObj(container_name) {
            ...
            container.find('.well').each(function (index) {
        ...
        function bpFieldInitRepeatableElement(element) {
            ...
            if (element.val()) {
                ...
                let values = element.val() != 'null' ? element.val() :  "[]";

                var repeatable_fields_values = JSON.parse(values);
        ...
        function newRepeatableElement(container, field_group, values) {
            ...
            var container_holder = $('[data-repeatable-holder='+field_name+']');
             ...
             // Access the name from the hairItems array
             if (hairItems[h]) {
                var hairItemName = hairItems[h].name; // Get the name from the hairItems array
                var hairItemImage = hairItems[h].image; // Get the name from the hairItems array

                var html = '<div class="col-md-4">';
                html += '<img src="/' + hairItemImage + '" width="80" />';
                html += '<span>' + hairItemName + '</span>';
                html += '</div>';
                container_holder.append(html);
