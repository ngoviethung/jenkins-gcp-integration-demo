...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            function getMinMaxElo() {

                $('input[name="min_elo_score"]').val('');
                $('input[name="max_elo_score"]').val('');

                let typeId = $('select[name="type_id"]').val();
                let topicId = $('select[name="topic_id"]').val();

                if(typeId && topicId) {
                    $.ajax({
                        method: 'POST',
                        url: '{{ url('admin/eloconvert/getMinMax') }}',
                        data: {
                            typeId: typeId,
                            topicId: topicId,
                        },
                        success: function (result) {
                            ...
                            if(result.code === 200) {
                                $('input[name="min_elo_score"]').val(result.data.min_score);
                                $('input[name="max_elo_score"]').val(result.data.max_score);
                            }
                        }
                    })
                }
